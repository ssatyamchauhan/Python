...
def get_movie_list_details(moviedetails):
	for i in moviedetails:
		...
		for i in url:
			if i=="/":
				...
				string=string+i
		filename=(os.path.join("/home/satya/Desktop/Webscraping/json_files/",string+".json"))
		exists=(path.exists(filename))
		if exists:
			with open(filename) as f :
				var=json.load(f)
				list_of_250_movies.append(var)

		else:
			arg=scrap_movie_details(url)
	...
	a=json.dumps(list_of_250_movies)
	with open("/home/satya/Desktop/Webscraping/json_files/totalmovielist.json","w") as f:
		f.write(a)
top_ten_movie = task1[:250]
